...
public class MascotaDAO {
    ...
    public static List<Mascota> listarTodos() throws SQLException {
        ...
        try (Statement st = conn.createStatement();
            ...
            while (rs.next()) {
                lista.add(new Mascota( // Usa constructor con ID
                        rs.getString("id"), rs.getString("nombre"), rs.getString("especie"),
                        rs.getString("raza"), rs.getInt("edad"), rs.getString("propietarioDocumento")
                ));
            }
        }
        return lista;
